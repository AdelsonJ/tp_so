...
Gerenciador* novoGerenciador() {
    ...
    // Inicializa listas de processos bloqueados, finalizados e em espera
...
void bloquear(Gerenciador *pm, int id_processo) {
    ...
    // Devolve uma mensagem de texto se não encontrar
...
ProcessoSimulado* copiarProcesso(ProcessoSimulado *pai, int desvio) {
    ...
    p->bloqueio = 0;
    p->completo = 0;
    p->tempo_inicio = pai->tempo_inicio;
    return p;
}

// Função que muda o processo que esta executando
void adicionaNovoExecutando(Gerenciador *pm){
    // Pega o primeiro processo da fila de espera, retira-o de lá e o coloca pra execução
...
int executarInstrucao(ProcessoSimulado *p, Gerenciador *pm) {
    ...
    // Parse a instrução
    if (sscanf(instrucao, "S %d", &valor) == 1) { // Atrubui um valor para a variavel
        p->var = valor;
    } else if (sscanf(instrucao, "A %d", &valor) == 1) { // Soma um valor para a variavel
        p->var += valor;
    } else if (sscanf(instrucao, "D %d", &valor) == 1) { // Subtrai um valor para a variavel
        p->var -= valor;
    } else if (strcmp(instrucao, "B") == 0) { // Bloqueia o processo
        bloquear(pm, p->id);
        p->contP++;
        p->bloqueio = 1;
        return 2;
    } else if (strcmp(instrucao, "E") == 0) { // Termina a simulação
        p->completo = 1; 
        return 1;
    } else if (sscanf(instrucao, "F %d", &valor) == 1) { // Cria um novo processo
        ...
        p->contP++;
        return 3;
    } else if (sscanf(instrucao, "R %s", nome_arquivo) == 1) { // Substitui as intruções atuais pelas lidas em um arquivo
        ...
        if (arquivo) {
            ...
            return 0;
        } else { // Instrução não incluida no escopo do programa
...
// Função que faz a execução de cada parte da instrução enquanto ela existir e o tiver processos disponiveis
void simular(Gerenciador *pm) {
    int estado = 0;
    // Se o processo executavel estiver vazio, preencha-o com o primeiro elemento da fila de espera, se houver
    if(pm->processo_execucao == NULL){
        ...
        // Se o tempo não tiver sido marcado ainda, marque-o agora
        if(p->tempo_inicio == -1)
...
void processManager(int read_pipe, const char *nome_arquivo) {
    ...
    // Cria o primeiro processo e o coloca aquele que vai executar
